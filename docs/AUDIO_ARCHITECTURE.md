...
Device list, alive, sample rate, buffer, data source and jack changes invalidate a running configuration. Stop both directions and show why; selected UIDs remain visible as missing. Never substitute default devices. Phase 1 requires explicit restart after the user checks wiring; automatic reconnection is deferred to Phase 6. Re-enumeration resolves volatile IDs from stable UIDs.
