...
* **Native UI execution was blocked locally.** The initial six XCUITest cases compiled, but the runner timed out while enabling automation mode before any UI case executed. Native computer-use inspection of the runner also timed out. This is not a local UI pass. Log: `artifacts/readiness-ui.log`.
...
[GitHub ARM64 macOS run 34007640194](https://github.com/CyberJorsh/XboxVoiceDeck/actions/runs/34007640194) passed on source commit `152e3b0cd7ed623879c74386f87104777d2a7569`: Release build, all 57 unit tests, the clock/fault/concurrency schedules and sanitizers, six silent safety checks, 26 probe checks, and **all seven native UI tests with zero failures**. The current Release was also built and signature-verified locally; the Debug fixture environment keys and implementation markers are absent from its executable. Local log: `artifacts/readiness-delivery-release.log`.

The UI suite verified missing-device and permission-denied flows, preflight with muted start/stop, profile review and muted restoration, tone confirmation/cancellation and bypass, the silent check and copied diagnostics, plus window close/relaunch. It found a minimum-window-height issue that placed emergency bypass outside the usable desktop; the app now opens a smaller single control window and keeps its tab content scrollable. Closing the window quits the app and invokes synchronous routing shutdown. Test launch settings use Debug-only environment values, and scrolling checks require controls to fit fully inside the viewport before a click.

These hosted UI tests use explicitly simulated services and open no audio devices. They validate native controls and the app model, not real permission dialogs, analog audio, driver shutdown, Xbox reception or M1 hardware acceptance. Hosted test bundles are retained by CI for seven days; no development-host audio or device inventory was uploaded.

## Microphone permission repair (0.3.1)

The testing Mac reported authorization `0` (not determined) together with duplicate capture-device selection. Source inspection and a regression test reproduce the cause: routing validation returned before requesting permission, while the generic error banner incorrectly offered Microphone settings. Permission now has an independent action, and the two physical preflight checks identify themselves as manual observations. See [MICROPHONE_PERMISSION.md](MICROPHONE_PERMISSION.md) for the retest steps.
...
Local validation: **78 XCTest cases passed** (30 kernel/configuration/safety, 30 routing-model lifecycle, eight preflight/report, five endpoint-kernel and five endpoint-model cases). Release build and signature verification passed. Existing clock/fault/concurrency simulations passed. An additional 500-cycle endpoint render/cancel/snapshot stress and oversized-callback silence check passed normally, under ASan/UBSan and under TSan. Logs: `artifacts/endpoint-cancel-tests.log` and `artifacts/endpoint-cancel-release.log` (ignored). The native UI suite adds input and output test-button flows for execution in hosted CI; its responses are simulated.
...
Startup cancellation is latched before control-queue teardown: the callback reads an independent atomic cancellation flag, so Stop/Bypass during HAL initialization cannot emit a later tone. The regression includes cancellation before the first callback and concurrent cancellation by both mechanisms. The standalone routing probe also passes its 26 checks after separating the engine interface from app services; six silent safety checks pass. The test script supports macOS system Bash.

## 0.4.1 hardware-readiness regression pass

Local development-host evidence, 2026-09-06; physical M1/Xbox acceptance remains pending:

- ARM64 Debug and Release builds pass; deployment target remains macOS 14. Version is **0.4.1 (6)**, with the existing bundle identifier and microphone entitlement.
- **95 unit tests pass**, including delayed buffer acknowledgement, notification/write ordering, timeout/listener cleanup, uncertain rollback acknowledgement, per-device inventory isolation, selected-device failure, permission revocation before/during startup completion, coalesced discovery, stale results, discovery timeout/recovery, real engine mute/stop with its control queue deliberately blocked, cancellation generations, system/alert-output collisions and latency warnings.
- **13 native UI fixture tests pass**, including Cancel startup, output warnings, permission flows, independent input/output tests and calibration controls. Fixtures open no hardware audio streams.
- Ten healthy clock scenarios, two forced-fault recoveries and concurrency stress pass. Address/undefined-behavior and thread sanitizer kernel runs pass, including 500 concurrent endpoint cancellation cycles per configuration. These sanitizer results cover the C kernel, not all Swift/driver behavior.
- The six offline safety checks and 26 routing-probe argument/configuration checks pass. The real device inventory is readable on this development Mac; no Xbox/USB/HyperX topology is available here.
- Local evidence: `artifacts/readiness-fix-build.log`, `artifacts/readiness-fix-tests.log`, `artifacts/readiness-fix-ui.log`, `artifacts/readiness-fix-preparation.log`, and timestamped `build/tests/*.xcresult` bundles. These files are intentionally excluded from Git.

No microphone capture, audible output, controller reception, third-party driver stall, electrical compatibility or physical latency was certified by this pass. Follow the [first-session checklist](READINESS.md#first-session-with-041-6). A driver that never returns can still prevent HAL teardown; immediate app-side safety cancellation does not stop audio generated by other applications on the same device.
