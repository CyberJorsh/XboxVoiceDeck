...
Do not advance to voice effects or soundboard development until both physical directions pass. A later development-host session exercised USB devices and the headset-jack input, but full hardware acceptance remains pending. See the [live troubleshooting guide and session findings](LIVE_TROUBLESHOOTING.md) for source selection, gain, buzz and gameplay-return checks.
