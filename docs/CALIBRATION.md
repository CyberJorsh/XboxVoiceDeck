...
* Completion, Cancel, mute, bypass, route resync/underrun or a callback error cancels the tone and latches Xbox output mute. Ordinary level edits cannot clear that mute. Explicit user unmute is required to resume the microphone.
* Switching away from the calibration page or app cancels the tone. Sleep stops running routing; startup remains muted and requires explicit restart. Stop/quit cancels tone state before Audio Unit disposal.
